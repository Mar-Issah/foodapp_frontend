...
const OrdersPage = () => {
  ...
  return (
    <>
      <div className='bg-custom-blueblack w-screen min-h-screen'>
        <Navbar />
        <div className='px-4 lg:px-10 xl:px-40 pb-14 w-screen'>
          {orders?.length === 0 && (
          ...
          {isLoading ? (
            <div className='flex justify-center items-center w-screen h-64 pb-2'>
              <button
                type='button'
                className='bg-custom-orange my-10 mx-auto p-4 flex items-center animate-bounce shadow-lg rounded-lg'
                disabled
              >
                Please wait
              </button>
            ...
            <table className='text-sm w-full border-separate border-spacing-3 md:text-md lg:text-lg'>
              ...
              <tbody className='text-gray-100 overflow-y-auto'>
                {orders?.map((order) => (
